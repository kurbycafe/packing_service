...
public class ProductController {
    ...
    public void updateProductList() throws IOException, InterruptedException {
        ...
        int perPage = 100; // 첫 페이지만
        ObjectMapper objectMapper = new ObjectMapper();
        HttpClient httpClient = HttpClient.newHttpClient();

        String url = String.format("%s?consumer_key=%s&consumer_secret=%s&per_page=%d&page=1",
                baseUrl, consumerKey, consumerSecret, perPage);
        ...
        for (Map<String, Object> productMap : productList) {
            ...
            if (metaData != null) {
                for (Map<String, Object> meta : metaData) {
                    ...
                    if ("_j79_wcxd_expiries".equals(key) && !value.isEmpty()) {
                        String[] entries = value.split(",");
                        for (String entry : entries) {
                            String[] parts = entry.trim().split("x");
                            if (parts.length == 2) {
                                LocalDate expiryDate = LocalDate.parse(parts[0].trim());
    ...
    private double getTaxRate(String taxClass) {
        if (taxClass == null || taxClass.isEmpty() || taxClass.equals("0")) {
            return 1.0;
        }
        switch (taxClass) {
            case "7": return 1.07;
